...
public class AIControlledNPC {
    ...
    // Simu temp (à remplacer par les vraies données de l'API)
    ...
    private void performJump() {
        logger.at(Level.INFO).log("Le PNJ saute !");
    }
    ...
    private void simulateStateChanges(String action) {
        if ("jump".equals(action)) {
            ...
            logger.at(Level.FINE).log("Faim: " + currentHunger);
        }
        ...
        if ("idle".equals(action)) {
            ...
            logger.at(Level.FINE).log("Faim récupérée: " + currentHunger);
        }
    }
